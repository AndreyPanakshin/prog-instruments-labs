...
def test_one_sample_ttest():
    """Test one-sample t-test against population mean μ=50.

    Sample is symmetric around 50 → t ≈ 0, p > 0.05.
    """
    sample = np.array([49, 50, 51, 48, 52, 50, 49, 51, 50, 50])
    pop_mean = 50.0
    t_stat, p_val = ttest_1samp(sample, pop_mean)
    assert np.isclose(t_stat, 0.0, atol=1e-6)
    assert p_val > 0.05


def test_two_sample_ttest_equal():
    """Test two-sample t-test on equal-mean groups (α = 0.01).

    Both groups sampled from N(50, 2) → no significant difference expected.
    """
    np.random.seed(1)
