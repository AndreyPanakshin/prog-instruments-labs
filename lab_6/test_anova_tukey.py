"""Tests for ANOVA and Tukey HSD statistical procedures."""
...
@pytest.mark.parametrize(
    "group_means,common_std,expected_significant",
    [
        ([50, 50, 50], 2.0, False),
        ([50, 55, 60], 1.0, True),
        ([50, 52, 54], 5.0, False),
...
def test_anova_significance_logic(group_means, common_std, expected_significant):
    """Test ANOVA's ability to detect significant differences in group means.

    Parameters
    ----------
    group_means : list of float
        Expected means of simulated groups.
    common_std : float
        Shared standard deviation for all groups.
    expected_significant : bool
        Expected outcome of significance test (α = 0.01).
    """
    np.random.seed(42)
    groups = [
...
def test_tukey_called_only_when_anova_significant(mock_excel_data):
    """Ensure Tukey HSD is called only if ANOVA is significant (α = 0.01)."""
...
def test_tukey_not_called_when_anova_not_significant(mock_excel_data):
    """Ensure Tukey HSD is NOT called when ANOVA is not significant."""
    df = mock_excel_data.copy()
    region_data = df.iloc[2:]
    alpha = 0.01
